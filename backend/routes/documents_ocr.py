...
#     uploaded_at: str
#     ocr_processed: Optional[bool] = None  # Add OCR indicator
...
#     chunk_count: int
#     analysis_method: str
#     ocr_processed: bool  # Add OCR indicator
#     analysis: dict
...
#             # Capture original text length to detect if OCR was used
#             import io
#             import sys
...
#             # Check if OCR was used by looking at the console output
#             output = mystdout.getvalue()
#             sys.stdout = old_stdout
            
#             if "Attempting OCR extraction" in output or "OCR extraction complete" in output:
#                 ocr_processed = True
#                 text_extraction_method = "ocr"
#                 print(f"Document processed using OCR - extracted {len(text.split())} words")
#             else:
#                 print(f"Document processed using standard extraction - {len(text.split())} words")
                
#         else:  # .docx
#             text = extract_text_from_docx(file_bytes)
#             print(f"Text from DOCX: {len(text.split())} words extracted")
        
#         if not text.strip():
#             raise HTTPException(
#                 status_code=status.HTTP_400_BAD_REQUEST,
#                 detail="No text found in the document. The file may be corrupted or in an unsupported format."
#             )
...
#         # Store document in database with OCR indicator
...
#             analysis_method=f"{analysis.get('analysis_method', 'single')}_{text_extraction_method}",  # Include extraction method
...
#         print(f"Text extraction method: {text_extraction_method}, OCR processed: {ocr_processed}")
...
#             analysis_method=f"{analysis.get('analysis_method', 'single')}_{text_extraction_method}",
#             ocr_processed=ocr_processed,  # Include OCR status
#             analysis=analysis,
...
#         # Check if it's an OCR-specific error
#         if "tesseract" in str(e).lower() or "ocr" in str(e).lower():
#             raise HTTPException(
#                 status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
#                 detail=f"OCR processing failed: {str(e)}. Please ensure the file contains valid images."
#             )
#         else:
#             raise HTTPException(
#                 status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
#             swot_analysis=json.dumps(analysis.get("swot_analysis", {})),
#             word_count=word_count,
#             analysis_method=f"{analysis.get('analysis_method', 'single')}_text",  # Indicate text input
...
#             word_count=word_count,
#             chunk_count=len(chunks),
#             analysis_method=f"{analysis.get('analysis_method', 'single')}_text",
#             ocr_processed=False,  # Text input is never OCR
#             analysis=analysis,
#             document_text=text, # Add document_text to the response
#             analyzed_at=datetime.now().isoformat()
#         )
...
#             ocr_processed="_ocr" in doc.analysis_method if doc.analysis_method else False  # Check if OCR was used
#         )
...
#         "ocr_processed": "_ocr" in document.analysis_method if document.analysis_method else False,  # Include OCR status
